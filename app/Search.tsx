...
export function Search() {
  ...
  return <>
    <button className="search-trigger" onClick={() => setOpen(true)} aria-label="搜索知识库"><span>⌕</span><span className="search-label">搜索知识</span><kbd>⌘ K</kbd></button>
    {open && <div className="search-backdrop" onClick={() => setOpen(false)}>
      <div className="search-dialog" role="dialog" aria-modal="true" aria-label="搜索知识库" onClick={(e) => e.stopPropagation()}>
        <div className="search-input-wrap"><span>⌕</span><input autoFocus placeholder="搜索业务、技术或系统设计…" value={query} onChange={(e) => setQuery(e.target.value)} /><button onClick={() => setOpen(false)}>ESC</button></div>
